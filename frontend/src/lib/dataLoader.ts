...
function ensureStringArray(value: unknown): string[] {
  // If it's an array, flatten it out by splitting any items that contain commas
  if (Array.isArray(value)) {
    return value
      .filter((item): item is string => typeof item === "string")
      .flatMap((item) => item.split(",")) // <-- ADD THIS: Split comma-separated items
      .map((item) => item.trim())
      .filter(Boolean);
  }

  // If it's a single long string with commas, split it into an array
  if (typeof value === "string") {
    return value
      .split(",") // <-- ADD THIS: Split the string by commas
      .map((item) => item.trim())
      .filter(Boolean);
  }
