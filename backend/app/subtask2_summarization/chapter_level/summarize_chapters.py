...
# Generates short/medium/long summaries for every chapter.
# Handles ANY transcript length via automatic chunking.
# Now enhanced with captions and frame paths from subtask1_segmentation/captions/.
# """
import json
import logging
from pathlib import Path
...
from backend.app.config import INPUT_DIR, CHAPTER_SUM_DIR, CAPTIONS_DIR, FRAMES_DIR, CHUNK_CHAR_LIMIT
from backend.app.common.utils.llm_client import call_llm, parse_json_response, summarize_long_text
...
# ── Captions + Frames loading ─────────────────────────────────────────────────

def load_captions_for_video(video_id: str) -> dict:
    """
    Loads the captions JSON for a video from subtask1_segmentation/captions/.
    Returns a dict keyed by slide_id.
    Format: {slide_id: {slide_start, slide_end, caption, transcript, frame_path, embedding, ...}}
    """
    ...
    if not captions_files:
        logger.debug(f"No captions file found for {video_id} in {CAPTIONS_DIR}")
        return {}
    
    with open(captions_files[0], "r", encoding="utf-8") as f:
        data = json.load(f)
    
    # Handle both formats: list of slides or dict with "slides" key
    slides = data.get("slides", [])
    return {slide["slide_id"]: slide for slide in slides}


def get_frame_urls_for_chapter(video_id: str, start_time: float, end_time: float,
                               captions_data: dict, base_url: str = "") -> list[str]:
    """
    Returns frame image paths (or URLs if base_url provided) for slides
    that fall within a chapter's time range.
    Uses the frame_path field from the captions JSON.
    """
    frame_urls = []
    for slide in captions_data.values():
        ...
        if start_time <= ts <= end_time:
            ...
            if fp:
                if base_url:
                    # Try to make it a URL relative to /frames/
                    try:
                        frame_path = Path(fp)
                        if FRAMES_DIR in frame_path.parents or str(FRAMES_DIR) in str(fp):
                            rel = frame_path.relative_to(FRAMES_DIR)
                            frame_urls.append(f"{base_url}/frames/{rel}")
                        else:
                            frame_urls.append(f"{base_url}/frames/{frame_path.name}")
                    except Exception:
                        frame_urls.append(f"{base_url}/frames/{Path(fp).name}")
                else:
                    frame_urls.append(str(fp))
    return frame_urls


# def enrich_chapter_with_captions(chapter: dict, captions_data: dict) -> dict:
#     """
#     Adds slide-level captions and frame_paths from the captions JSON
#     into a chapter dict. This gives richer visual context than frame_captions alone.
#     """
...
#     matching_slides = [
#         slide for slide in captions_data.values()
#         if start <= slide.get("representative_timestamp", slide.get("slide_start", 0)) <= end
#     ]

#     # Pull frame paths from matching slides
#     frame_paths = [s.get("frame_path", "") for s in matching_slides if s.get("frame_path")]

#     # Pull slide-level captions (more detailed than chapter-level frame_captions)
#     slide_captions = [s.get("caption", "") for s in matching_slides if s.get("caption")]

#     # Merge with existing frame_captions — slide captions take priority if richer
#     existing_captions = chapter.get("frame_captions", [])
#     merged_captions = slide_captions if slide_captions else existing_captions

#     return {
#         **chapter,
#         "frame_captions": merged_captions,
#         "frame_paths":    frame_paths,
#         "slide_count":    len(matching_slides)
#     }

# In backend/app/subtask2_summarization/chapter_level/summarize_chapters.py
...
def enrich_chapter_with_captions(chapter: dict, captions_data: dict) -> dict:
    ...
    # ── Extract frame_paths and convert to relative paths ──────────────────
    frame_paths = []
    for slide in matching_slides:
        fp = slide.get("frame_path", "")
        if fp:
            # Convert absolute path to relative path from FRAMES_DIR
            fp_path = Path(fp)
            try:
                # If it's an absolute path, make it relative to FRAMES_DIR
                rel_path = fp_path.relative_to(FRAMES_DIR)
                frame_paths.append(str(rel_path))
            except ValueError:
                # If it's already relative or not under FRAMES_DIR, use as-is
                # But check if it starts with the FRAMES_DIR pattern
                if str(FRAMES_DIR) in str(fp_path):
                    # Extract the part after FRAMES_DIR
                    rel = str(fp_path).split(str(FRAMES_DIR))[-1].lstrip("/")
                    frame_paths.append(rel)
                else:
                    frame_paths.append(str(fp_path))
...
def _base_fields(chapter: dict) -> dict:
    return {
        ...
        "end_time":         chapter["end_time"],
        "duration_seconds": chapter["end_time"] - chapter["start_time"],
        "frame_paths":      chapter.get("frame_paths", []),
        "slide_count":      chapter.get("slide_count", 0),
    }
...
def normalize_video_json(json_path: Path, raw_data: dict) -> dict:
    """
    Supports:
      1. Mock format  — top-level video_id + chapters
      2. Student A format — metadata{} + chapters[]
    Also enriches chapters with captions/frames from CAPTIONS_DIR.
    """
    if "video_id" in raw_data and "chapters" in raw_data:
        video_id = raw_data["video_id"]
        captions_data = load_captions_for_video(video_id)
        if captions_data:
            logger.info(f"  Found captions data for {video_id} — enriching chapters with frame paths")
            raw_data["chapters"] = [
                enrich_chapter_with_captions(ch, captions_data)
    ...
    if "metadata" in raw_data and "chapters" in raw_data:
        meta = raw_data["metadata"]
        stem = json_path.stem
        video_id = stem.split("_chapters")[0]
        video_id = video_id.replace("_transcripts", "").replace("_captions", "")

        duration = int(meta.get("video_duration_seconds", 0))
        chapters = raw_data["chapters"]

        # Enrich chapters with captions + frame paths from captions folder
        captions_data = load_captions_for_video(video_id)
        if captions_data:
            logger.info(f"  Found captions data for {video_id} — enriching chapters with frame paths")
            chapters = [enrich_chapter_with_captions(ch, captions_data) for ch in chapters]
        else:
            logger.debug(f"  No captions data found for {video_id}")
    ...
    for path in files:
        try:
            with open(path, "r", encoding="utf-8") as f:
                peek = json.load(f)
            video_id = peek.get("video_id") or path.stem.split("_chapters")[0]
        except Exception:
            video_id = path.stem

        cached = CHAPTER_SUM_DIR / f"{video_id}_chapter_summaries.json"
        if cached.exists():
            logger.info(f"Skipping {video_id} — already summarized")
            with open(cached) as f:
                results.append(json.load(f))
            continue
